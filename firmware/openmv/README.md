...
- VIS/上传仍使用保守的 `0.65` 连续三帧判定。WARNING/CRITICAL 黄灯
  状态另有仅用于板载灯的高召回判定：现场实测真人原始分数
  `0.5508..0.6445`，所以使用 `0.54` 且最近三帧二次命中即可由黄转红；
  该结果不会写入 VIS、ESP32 本地报警、服务器样本或训练数据。
- IDE 画面与每 2 秒一条的 `VISION_SCORE` 日志会保留真实原始分数，即使
  保守 VIS 判定仍为 `person=0`，便于继续做空景/真人标定。
