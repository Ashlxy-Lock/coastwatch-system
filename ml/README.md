...
这不是海啸、风暴潮或航海预报模型。标签来自项目内版本化的波高/风速演示规则，不是事故真值或官方安全阈值。STM32 的本地确定性规则仍是最终报警路径；模型先以旁路（shadow）方式运行。
...
公网网关仍要求 `X-Device-Token` 或 Bearer Token。返回值会同时给出环境模型等级、STM32 本地等级、最终较高等级、数据质量、降级状态和原因码。模型文件缺失或损坏时，服务不会瘫痪，而是明确返回 `model_source: rule-fallback`。
